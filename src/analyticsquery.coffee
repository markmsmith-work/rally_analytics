...
class AnalyticsQuery
  ...
  Which you can then use when instantiating a query.
      ...
      query = new rally_analytics.AnalyticsQuery(config, XHRMock)
  ...
  * **allWarnings** NOT YET IMPLEMENTED
  ...
  constructor: (config, @_XHRClass) ->
    ...
    @_sort = {_ValidFrom: 1}
    @_startIndex = 0
    @_pageSize = 10000  # Start with a really large number but set it to whatever comes back on the first request
    @_callback = null
...
class GuidedAnalyticsQuery extends AnalyticsQuery
  ...
  for composing queries. Like the raw AnalyticsQuery, you start by creating a GuidedAnalyticsQuery Object. 
      ...
      query = new rally_analytics.GuidedAnalyticsQuery(config, XHRMock)
...
class AtAnalyticsQuery extends GuidedAnalyticsQuery
  ...
  This pattern will tell you what a set of Artfacts looked like at particular moments in time
      ...
      query = new rally_analytics.AtAnalyticsQuery(config, XHRMock, '2012-01-01T12:34:56.789Z')
  ...
  ###
  constructor: (config, @_XHRClass, zuluDateString) ->
    super(config, @_XHRClass)
...
class AtArrayAnalyticsQuery extends GuidedAnalyticsQuery
  ...
  `query = new rally_analytics.AtArrayAnalyticsQuery(config, XHRMock, ['2012-01-01T12:34:56.789Z', '2012-01-02T12:34:56.789Z', ...])`
...
class BetweenAnalyticsQuery extends GuidedAnalyticsQuery
  ...
  use this approach. Note: the 'AtArray' approach will supercede this for time-series charts at some point in the future.
      ...
      query = new rally_analytics.BetweenAnalyticsQuery(config, XHRMock, '2012-01-01T12:34:56.789Z', '2012-01-10T12:34:56.789Z')
  ...
  It will expand to a query like this:
      ...
      # }
  ###
  constructor: (config, @_XHRClass, zuluDateString1, zuluDateString2) ->
    super(config, @_XHRClass)
...
class TransitionsAnalyticsQuery extends GuidedAnalyticsQuery
  ...
  !TODO: Indent below to make sure it works and add example
  query = new TransitionsAnalyticsQuery(config, XHRMock,
  ...
  !TODO: Indent below to make sure it works and add example
  query = new rally_analytics.TransitionsAnalyticsQuery(config, XHRMock,
